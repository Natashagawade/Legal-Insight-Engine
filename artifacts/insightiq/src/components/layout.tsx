...
import { Scale, History, LayoutDashboard, FileSearch2, GraduationCap, Settings } from "lucide-react";
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { href: "/general",  label: "General",          icon: FileSearch2 },
          ...
          <div className="flex items-center gap-3">
            <Link href="/">
              <div className="flex items-center gap-3 cursor-pointer">
                ...
                <span className="font-display font-bold text-xl tracking-tight text-white hover:text-white/80 transition-colors">
                  InsightIQ
                </span>
              </div>
            </Link>
              ...
              return (
                <Link key={item.href} href={item.href}
                  className={cn(
                    ...
                    isActive ? "bg-white/8 text-white" : "text-muted-foreground hover:text-white hover:bg-white/5"
                  )}>
